...
# TASKS_DIR = get_sync_dir('lyx/tasks')



TRAIN_VOCAB = 'lyx/R2R/data/train_vocab.txt'
TRAINVAL_VOCAB = 'lyx/R2R/data/trainval_vocab.txt'

IMAGENET_FEATURES = 'lyx/img_features/ResNet-152-imagenet.tsv'
PLACE365_FEATURES = 'lyx/img_features/ResNet-152-places365.tsv'

train_vocab = get_sync_dir(TRAIN_VOCAB)
trainval_vocab = get_sync_dir(TRAINVAL_VOCAB)
print(train_vocab)
...
def train(train_env, tok, n_iters, log_every=100, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        writer.add_scalar("loss/spe_loss", aux_loss1, idx)
        writer.add_scalar("loss/pro_loss", aux_loss2, idx)
        writer.add_scalar("loss/mat_loss", aux_loss3, idx)
        writer.add_scalar("loss/fea_loss", aux_loss4, idx)
        writer.add_scalar("loss/ang_loss", aux_loss5, idx)
        ...
        loss_str += ', spe_weight: %.1f' % (args.aux_speaker_weight)
        loss_str += ', pro_weight: %.1f' % (args.aux_progress_weight)
        loss_str += ', mat_weight: %.1f' % (args.aux_matching_weight)
        loss_str += ', ang_weight: %.1f' % (args.aux_angle_weight)
        loss_str += ', fea_weight: %.1f' % (args.aux_feature_weight)
        loss_str += '\n'
